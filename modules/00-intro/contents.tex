...
\begin{frame}
  \begin{question}
    What are the \emph{research} methods we (in the security community) use and 
    why?
  \end{question}
\end{frame}
...
\begin{frame}<presentation>
  \begin{block}{The goal}
    \begin{itemize}
      \item Give a holistic view of \emph{Science} of Security.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  \begin{example}\label{SoKProblem1}
      \textcquote[\S IV]{SecurityAsAScience}{%
        \textins*{C}laims of necessary conditions for real-world security are 
        unfalsifiable.
  ...
  \begin{example}\label{SoKProblem2}
    \textcquote{SecurityAsAScience}{%
      Unfalsifiable claims are common in security---and they, along with 
      circular arguments, are used to justify many defensive measures 
      \textelp{}
      \textins{T}here are many ways to argue measures in, but no way to argue 
      one out.%
    }
  \end{example}
\end{frame}
...
\begin{frame}<presentation>
  \begin{block}{Aim of Science of Security course}
    \begin{itemize}
      \item Complement the general methods course (TAMOS).
      \item Better prepare students for thesis (and hopefully worklife \dots).
      \item They should be able to contribute to \emph{scientifically} based 
  ...
  \begin{remark}
    \begin{itemize}
      \item This course is now fully synced with TAMOS\@.
    \end{itemize}
  \end{remark}
\end{frame}
...
In the area of cybersecurity, the student should be able to\footnote{%
  These are the relevant learning outcomes from the (equivalent) courses DD2395 
  and DD2391 at KTH\@.
}:
\begin{itemize}
  \item \emph{identify} threats against confidentiality, integrity and 
    availability in digital systems;
  \item \emph{explain} basic terminology and concepts in computer security and 
    \emph{use} them;
  \item \emph{analyse} simple program code and systems (based on given or 
    self-made system descriptions) to \emph{identify} vulnerabilities and 
    predict corresponding threats;
  \item \emph{suggest} countermeasures against identified threats and 
    \emph{argue} for their suitability;
  \item \emph{compare} countermeasures and \emph{evaluate} their side effects;
  \item \emph{apply} countermeasures, \emph{present} and \emph{explain} their 
    reasoning to others.
\end{itemize}

In the area of the cybersecurity engineer's role in society, the student should 
be able to\footnote{%
  These are the relevant learning outcomes from the course DD2303 at KTH\@.
}:
\begin{itemize}
  \item \emph{analyse} and \emph{discuss} how the use and development of 
    digital systems and in particular the security of these systems affect and 
    are affected by social, economic, environmental, work environmental and 
    ethical sustainability as well as diversity, gender equality and equal 
    conditions;
  \item \emph{plan} and \emph{carry out} assignments within given time frames 
    and using available resources;
  \item \emph{write} short, clear and arguing texts based on own analysis as 
    well as given material.
\end{itemize}
...
of science, both orally as well as in writing, be able to\footnote{%
  These are the relevant learning outcomes from the course AK2030 at KTH\@.
}:
\begin{itemize}
  \item \emph{identify} definitions and descriptions of concepts, theories and 
    problem areas, as well as \emph{identify} the correct application of these 
    concepts and theories;
  \item \emph{account} for concepts, theories and general problem areas, as well 
    as \emph{apply} concepts and theories to specific cases;
  \item critically \emph{discuss} the definitions and applications of concepts 
    and theories as they applies to specific cases of scientific research.
\end{itemize}
...
\begin{frame}
  ...
  \begin{remark}
    \begin{itemize}
      \item The focus is on \emph{research methods}.
      \item It's \emph{not} on {constructing a secure solution}.
      \item We want to know \emph{how do you know if it is secure?}
    \end{itemize}
  \end{remark}
\end{frame}
    ...
    \begin{column}{0.5\columnwidth}
      \begin{block}{Teaching material}
        \begin{itemize}
          ...
          \item<2-3> Reading assignments with social annotation.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.5\columnwidth}
...
\begin{frame}
  \begin{block}{Teaching}<+>
    \begin{itemize}
      ...
      \item Write reflection to prepare for seminar.
  ...
  \begin{block}{Assessment}<+>
    Either
    \begin{enumerate}
      \item You do all prep and participate in all seminars.
      \item You do the project, write a report, peer review and participate in 
        the final seminar.
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}
  \begin{block}{Giving the course}
    \begin{itemize}
      \item Given twice pre year, for now: period 2 and 4.
      \item If you don't finish, you can try again next round.
    \end{itemize}
  \end{block}
\end{frame}
}
...
The first part is to read \citetitle{HowToDesignSecurityExperiments} by 
...
\citetitle{HowToDesignSecurityExperiments} and the TAMOS lectures.

\begin{frame}<presentation>
  \begin{block}{Scientificity, part 1}
    \begin{itemize}
      \item \fullcite{HowToDesignSecurityExperiments}
...
\paragraph{Models: Cryptography, formal methods, simulations}

\begin{frame}<presentation>
  \begin{block}{Models---deductive methods}
    \begin{itemize}
      \item Cryptography: proving crypto protocols (RSA and ElGamal).
      \item How models are inaccurate (side channels).
      \item Formal methods: proving complex protocols, software, hardware.
      \item Simulations: can't trial-and-error in real world (Tor).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}<presentation>
  \begin{block}{Statistics}
    \begin{itemize}
      \item How is statistics used in security?
      \item It's used in models and simulations (the science).
      \item Used in security solutions (or attacks).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}<presentation>
  \begin{block}{Qualitative methods}
    \begin{itemize}
      \item How are qualitative methods used in security?
      \item Qualitative methods are used to understand the human aspects of 
        security.
      \item It's a method to explore things, to explain.
      \item It can generate hypotheses that can be tested quantitatively.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}<presentation>
  \begin{block}{Ethics}
    \begin{itemize}
      \item What should be do and not do?
      \item What research questions should we answer and how can we do it?
    \end{itemize}
  \end{block}
\end{frame}
