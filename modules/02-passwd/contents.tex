...
The goal is to see how it compares to the systematic literature review.
...
Considering the paper by \citeauthor{SLRinIS} is from 2015, the tools are 
\enquote{traditional} tools---as opposed to \enquote{modern} tools that also 
use AI and large language models\footnote{%
  During a seminar, the library will tell us more about both traditional and 
  modern tools.
}.
Despite that the AI-based tools are cool and give good results, we still need 
the traditional tools.
Basically none of the AI-based tools are reproducible:
they might be biased towards the users interests,
the models might be updated (improved) and thus give different results,
and we don't know how the model arrives at the results.
The traditional tools on the other hand are reproducible:
they are fully deterministic---they don't do synonyms or similarity, only 
exactly what you specify---and will give the same results every time.
(Of course, they'll give new results when new papers are published, but that 
can be controlled by filtering---limiting the inclusion of publications between 
given dates, which is why it's important to document such data.)
